...
{
    // 确定锚点
    NSLog(@"self.imageView.layer.anchorPoint = %@",@(self.imageView.layer.anchorPoint));
    CGPoint point1 = CGPointMake(self.imageView.frame.origin.x + self.imageView.frame.size.width, self.imageView.frame.origin.y);
    CGPoint anchoPoint = self.imageView.center;
    CGPoint goalPoint = pointRotatedAroundAnchorPoint(point1, anchoPoint, -M_PI_2);
    CGRect virtualFrame = CGRectMake(goalPoint.x, goalPoint.y, self.imageView.frame.size.width, self.imageView.frame.size.height);
    CGPoint virtualCenter = CGPointMake(virtualFrame.origin.x + virtualFrame.size.width/2, virtualFrame.origin.y + virtualFrame.size.height/2);
    CGFloat ratioX = -0.5*(virtualCenter.x - anchoPoint.x)/(virtualFrame.size.width/2);
    CGFloat ratioY = -0.5 * (virtualCenter.y - anchoPoint.y)/(self.imageView.frame.size.height/2);
    CGPoint newAnchorPoint = CGPointMake(0.5 + ratioX, 0.5 + ratioY);
    self.imageView.layer.anchorPoint = newAnchorPoint;
    CATransform3D rotatedTransform = self.imageView.layer.transform;
    rotatedTransform = CATransform3DMakeRotation(M_PI_2, 0, 0, 1.0);//rotate为旋转弧度，绕Z轴转
...
static CGPoint pointRotatedAroundAnchorPointFunction(CGPoint point,CGPoint anchorPoint,CGFloat angle)
{
    return CGPointMake((point.x-anchorPoint.x)*cos(angle) - (point.y-anchorPoint.y)*sin(angle) + anchorPoint.x, (point.x-anchorPoint.x)*sin(angle) + (point.y-anchorPoint.y)*cos(angle)+anchorPoint.y);
}
