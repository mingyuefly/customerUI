...
{
    GMSheetAction *action = [[self alloc] init];
    if (action) {
        action.title = title;
        action.block = hander;
    }
...
@interface GMActionSheetViewController ()<UITableViewDelegate, UITableViewDataSource,UIGestureRecognizerDelegate>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray<GMSheetAction *> *sourceArray;
@property (nonatomic, strong) NSMutableArray<GMSheetActionBlock> *blockArray;
@property (nonatomic, strong) NSMutableArray<GMActionSheetCellModel *> *modelArray;
@property (nonatomic, assign) BOOL tableViewVisible;

@end

@implementation GMActionSheetViewController
...
{
    GMActionSheetViewController *actionSheetViewController = [[GMActionSheetViewController alloc] init];
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, DEVICE_HEIGHT, DEVICE_WIDTH, GMLAYOUTRATE(3 * 54 + 6)) style:UITableViewStylePlain];
...
-(NSMutableArray<GMSheetAction *> *)sourceArray
{
    if (!_sourceArray) {
...
-(NSMutableArray<GMActionSheetCellModel *> *)modelArray
{
    if (!_modelArray) {
...
-(NSMutableArray<GMSheetActionBlock> *)blockArray
{
    if (!_blockArray) {
...
-(void)setTableViewVisible:(BOOL)tableViewVisible
{
    if (tableViewVisible) {
        [UIView animateWithDuration:0.3 animations:^{
            self.tableView.transform = CGAffineTransformTranslate(self.tableView.transform, 0, -GMLAYOUTRATE(3 * 54 + 6));
        }];
    } else {
        [UIView animateWithDuration:0.3 animations:^{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        ...
        {
            [self popActionSheet];
            GMSheetActionBlock block = [self.blockArray objectAtIndex:indexPath.section * (self.blockArray.count - 1) + indexPath.row];
            GMSheetAction *action = [self.sourceArray objectAtIndex:indexPath.section * (self.sourceArray.count - 1) + indexPath.row];
            block(action);
        }
            break;
        case 1:
...
{
    GMActionSheetCell *cell = [GMActionSheetCell CellWithTableView:tableView];
...
-(void)addAction:(GMSheetAction *)action
{
    if (self.sourceArray.count > 0) {
        ...
        GMActionSheetCellModel *model = [[GMActionSheetCellModel alloc] init];
        ...
        if (action.block) {
            ...
            GMSheetActionBlock block = ^(GMSheetAction * action){};
